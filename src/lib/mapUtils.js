...
export const CHI_BOUNDS = [[41.20, -88.80], [42.60, -87.00]]
export const CHI_MIN_ZOOM = 9
export const CHI_MAX_ZOOM = 16

export const USA = { lat: 39.8283, lng: -98.5795 }
export const GLOBAL_ZOOM = 3

/* ---------- Interaction / UI constants ---------- */
export const INITIAL_RADIUS_MILES = 10
...
export function boundsForMiles(center, miles) {
  ...
  const { dLat, dLng } = squareMileDeltas(c.lat)
  return [
    [c.lat - (dLat * miles) / 2, c.lng - (dLng * miles) / 2],
    [c.lat + (dLat * miles) / 2, c.lng + (dLng * miles) / 2],
  ]
}
...
export const TEAM_COLOR = {           // ← export so UI can reuse
  cubs:     '#2a6ae0',
  whitesox: '#e8e8e8',
  other:    '#666666',
  chicago:  '#0ea5e9',
