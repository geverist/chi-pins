...
DROP POLICY IF EXISTS "Allow public update access to navigation settings" ON navigation_settings;
DROP POLICY IF EXISTS "Allow insert access to navigation settings" ON navigation_settings;
...
-- Allow insert access for initial setup
CREATE POLICY "Allow insert access to navigation settings"
  ON navigation_settings
  FOR INSERT
  WITH CHECK (true);
